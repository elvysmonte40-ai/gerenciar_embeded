...
import { p as passwordResetEmailTemplate, w as welcomeEmailTemplate } from './email-templates_C8YvwTUp.mjs';
...
const GET = async ({ request }) => {
  try {
    ...
    const { data: customTemplates } = await supabaseAdmin.from("email_templates").select("*").eq("organization_id", orgId);
    const templates = ["welcome", "password_reset"].map((type) => {
      ...
      return {
        ...
        description: type === "welcome" ? "Enviado quando um novo usuário é cadastrado no sistema." : "Enviado quando um usuário solicita a redefinição de senha."
      };
    });
...
const DELETE = async ({ request }) => {
  try {
    ...
    await supabaseAdmin.from("email_templates").delete().eq("organization_id", orgId).eq("template_type", templateType);
