...
const DepartmentSection = ({ group, isCollapsed, currentId }: { group: DepartmentGroup, isCollapsed: boolean, currentId: string | null }) => {
    // Check if any process in this group is active
    ...
    return (
        <div className={`w-full space-y-1 ${isCollapsed ? 'flex flex-col items-center border-t border-gray-100 pt-3 relative' : ''}`}>
            ...
            >
                {isCollapsed ? (
                    <div className="mb-1 relative">
                        <div className="h-6 w-6 rounded-md bg-gray-100 flex items-center justify-center text-gray-500 font-medium text-xs">
            ...
            {/* List of Processes - Conditional Rendering */}
            <div className={`
                space-y-1 
                ${!isCollapsed
                    ? `ml-2 border-l border-gray-200 pl-2 overflow-hidden transition-all duration-300 ease-in-out ${isOpen ? 'max-h-[1000px] opacity-100' : 'max-h-0 opacity-0'}`
                    : `w-full flex flex-col items-center space-y-2 overflow-hidden transition-all duration-300 ease-in-out ${isOpen ? 'max-h-[1000px] opacity-100 mt-2' : 'max-h-0 opacity-0 mt-0'}`
                }
            `}>
                        ...
                        return (
                            <a
                                ...
                                className={`block rounded-md transition-colors ${isCollapsed
                                    ...
                                    } ${(isCollapsed && isActive) ? 'bg-white text-brand border border-gray-200 shadow-sm' : ''}`}
                            >
                                {isCollapsed ? (
...
export function ProcessSidebar() {
    ...
    useEffect(() => {
        // Extract ID from path to highlight active process
        const pathParts = window.location.pathname.split('/');
        // Assuming path is like /processes/[id] or /processes/[id]/editar
        // If it's /processes or /processes/novo, currentId is null
        ...
        async function fetchData() {
            ...
            try {
                ...
                // Filter processes based on viewer_roles and editor_roles
                if (!isOrgAdmin) {
                    processes = processes.filter(process => {
                        const viewerRoles = process.viewer_roles || [];
                        const editorRoles = process.editor_roles || [];

                        if (viewerRoles.length === 0) return true; // Visible to everyone in org

                        // Check if user has a role that is in the viewer_roles or editor_roles list
                ...
                // Group by department
                const groupedMap = new Map<string, DepartmentGroup>();
    ...
    if (loading) {
        return <div className="p-4 text-xs text-text-secondary animate-pulse w-64">Carregando processos...</div>;
    }
