...
export const IndicatorCard: React.FC<IndicatorCardProps> = ({
  ...
  const realized =
    isAnnual && lastEntry?.accumulated
      ? lastEntry.accumulated.realized
      : lastEntry?.realized;
  const target =
    isAnnual && lastEntry?.accumulated
      ? lastEntry.accumulated.target
      : lastEntry?.target;
  const performance =
    isAnnual && lastEntry?.accumulated
      ? lastEntry.accumulated.performance
      : lastEntry?.performance;
  ...
  const TrendIcon =
    lastEntry?.status === "GREEN"
      ? TrendingUp
      : lastEntry?.status === "RED"
        ? TrendingDown
        : Minus;
